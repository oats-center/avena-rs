<script lang='ts'>
  ...
  interface Sensor {
    "cabinet_id" : string; //for accessing the config
    "labjack_serial" : string; //for accessing the config
    "connected_channel": number //for accessing the config
    "sensor_name" : string; //just a display name, no real importance
    "sensor_type" : string; //for the map
    "x_pos" : number; //for the map (0 <= x_pos <= 1)
    "y_pos" : number; //for the map (0 <= y_pos <= 1)
