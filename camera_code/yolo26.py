...
class CarCrossingTracker:
    def __init__(
        ...
        # target classes: merge these as "car"
        target_class_names=("car", "truck"),
        ...
        # fallback lane separators (3 horizontal bands)
        ...
        self.csv_writer.writerow(["obj_id", "lane_id", "timestamp_s", "x_cross_px", "vx_px_s", "speed_mph", "snapshot_path", "det_class"])
        ...
        # resolve target ids (car+truck) and treat as one category
        self.target_class_names = tuple(str(x) for x in target_class_names)
        ...
        # UI
        self.window_name = "Car/Truck Tracking"
        cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)

        print(f"[INFO] Model names: {self.names}")
        print(f"[INFO] Merging classes {self.target_class_names} => ids={sorted(self.target_class_ids)}")
    ...
    def _resolve_class_ids(self, class_names):
        ...
        for n in class_names:
            if n in name_to_id:
                ids.add(int(name_to_id[n]))
            else:
                print(f"[WARN] Class name '{n}' not found in model.names")
        if not ids:
            # fallback: accept nothing would break your pipeline; use 'car' if exists else 0
            fallback = name_to_id.get("car", 0)
            print(f"[WARN] No target classes found. Falling back to id={fallback}")
    ...
    def save_snapshot(self, frame_bgr, bbox_xyxy, obj_id, t_sec):
        ...
        snap_path = os.path.join(self.snap_dir, f"obj_{obj_id}_t{t_sec:.3f}.jpg")
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if results and len(results) > 0:
                ...
                if r.boxes is not None and r.boxes.id is not None:
                    ...
                    for box, tid, cls in zip(boxes, ids, clss):
                        # accept car OR truck (merged)
                        if int(cls) not in self.target_class_ids:
                            continue
                        x1, y1, x2, y2 = [float(v) for v in box]
                        cx = 0.5 * (x1 + x2)
                        cy = 0.5 * (y1 + y2)
                        dets.append((tid, int(cls), (x1, y1, x2, y2), (cx, cy)))
                        seen_this_frame.add(tid)

            # --- update KF + draw ---
            for tid, cls, bbox, (cx, cy) in dets:
                ...
                # LEFT->RIGHT CASE: track the FRONT (right edge x2)
                front_x = float(bbox[2])  # x2
                kf.update(front_x)

                self.last_bbox_by_id[tid] = bbox
                self.last_center_by_id[tid] = (cx, cy)
                self.last_cls_by_id[tid] = cls
                self.missed_by_id[tid] = 0

                vx = kf.vel()
                speed_mph = abs(vx) * self.PXPS_TO_MPH if vx is not None else 0.0

                # draw as "car" color (use car id if present else cls)
                car_color_id = 2 if "car" in (self.names.values() if isinstance(self.names, dict) else self.names) else cls
                col = colors(int(car_color_id), True)
                ...
                det_name = self.names[cls] if isinstance(self.names, dict) else self.names[int(cls)]
                label = f"car:{tid}  lane:{lane_id}  {speed_mph:.1f} mph"  # merged label
            ...
            for tid in seen_this_frame:
                ...
                cls = self.last_cls_by_id.get(tid, -1)
                ...
                if prev is not None:
                    ...
                    if prev_front < self.x_cross <= front:
                        ...
                        print(f"[EVENT] id={tid} ({det_name}->car) lane={lane_id} t={t_cross:.3f}s vx={vx:.2f}px/s mph={speed_mph:.1f}")
                        self.crossed_ids.add(tid)

                self.prev_front_by_id[tid] = (front, t_sec)

            cv2.putText(im0, f"objs={len(dets)}  kfs={len(self.kf_by_id)}  out={Path(self.save_dir).name}",
...
if __name__ == "__main__":
    tracker = CarCrossingTracker(
        # ORIGINAL YOLO26 weights (not your trained best.pt)
        model="yolo26n.pt",

        source="/Users/anuskadas/Desktop/Cars/vid1.mp4",
