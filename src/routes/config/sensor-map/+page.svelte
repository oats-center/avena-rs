<script lang='ts'>
  ...
  async function initialize(): Promise<void> {
    ...
    if(nats && selectedCabinet) {
      ...
      mapconfig = JSON.parse(tempConfig)
  ...
  onMount(() => {
    ...
    initialize().then(() => {console.log(sensors)});
    console.log(sensors)
  })
...
<div class="flex items-center h-screen">
  <div class="mx-40" style="position:relative ">
    <img 
    ...
    {#each sensors as sensor}
      {#if sensor.sensor_type === "Temperature"}
        ...
        style="position: absolute; top: {sensor.x_pos * 100}%; left: {sensor.y_pos * 100}%;"
        onclick={() => editingSensor = sensor}
        onkeydown={() => console.log(sensor)}
        role="button"
        tabindex=0
        >
          <path
...
<SaveModal {save_modal} {saveSensorChanges}/>
<CancelModal {cancel_modal} {handleCancel}/>
<DeleteModal {delete_modal} {deleteSensor}/>

<!--
  mapconfig = {
    "background" : HTMLImageElement,
    "labjackd.1.ch1" : {Sensor},
    "labjackd.1.ch2" : {Sensor},
    "labjackd.1.ch3" : {Sensor},
    "labjackd.1.ch4" : {Sensor},
    ...
  }
-->