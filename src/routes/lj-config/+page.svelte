<script lang="ts">
  ...
  let connectionId = generateRandomId();

  async function getSensorsList(): Promise<void> {
    try {
      const response = await fetch(`./nats/kv?connectionId=${connectionId}`);
      if(response.ok){
  ...
  async function getSensorValues(): Promise<void> {
    try {
      const url = `./nats/kv?type=getKVs&connectionId=${connectionId}`;
  ...
  async function watchValues(): Promise<void> {
    try {
      const url = `./nats/kv?type=watchVals&connectionId=${connectionId}`;
      const response = await fetch(url);
      if(response.ok){
        const result: KeyValueResponse = await response.json();
        sensorsList = result.sensorsList;
...
<div class="flex flex-col justify-center items-center">
  {#if sensorValues && updatedSensors}
    <div class="m-[5vw] mt-2 space-y-5">
