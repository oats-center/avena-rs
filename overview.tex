...
\begin{tikzpicture}[
  ...
  lbl/.style={font=\footnotesize, align=center, fill=white, inner sep=1.5pt, text=black!90},
...
\node[box, fill=green!10] (nats)    at (9.6,4.0)  {NATS JetStream\\subjects + KV config};

\node[box, fill=violet!10] (webapp)   at (0.0,1.6)  {SvelteKit webapp\\login, config, plots, export};
\node[box, fill=orange!12] (archiver) at (9.6,1.6)  {Rust \texttt{archiver}\\per-channel subscribers};

\node[box, fill=violet!10] (exporter) at (0.0,-0.8) {Rust \texttt{exporter}\\WS \texttt{/export} (CSV chunks)};
\node[box, fill=yellow!18] (parquet)  at (9.6,-0.8) {Parquet storage\\asset/day/channel partitions};

\draw[data] (labjack) -- node[lbl, above, yshift=1pt] {sensor reads} (streamer);
\draw[data] (streamer) -- (nats);
\node[lbl] at (7.5,3.82) {publish \texttt{Scan}};
\draw[cfg] (nats.north west) to[bend left=28] (streamer.north east);
\node[lbl] at (7.9,4.95) {KV config updates};
